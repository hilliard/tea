// Set up the UI/UX for the test case screens.  This object sets up all the functionality we need to:
//  1.  Bind to the "click" event of the "#addButton" on the modal form.
//  2.  Append data from the modal form to the Steps table.
...
var stepFieldsUI = {
    init: function() {
        ...
        $('#new-step-fields').validateOnBlur();

        $('#addButton').on('click', function(e) {
            // If the form validation on our Steps modal "form" fails, stop everything and prompt the user
            // to fix the issues.
            var isValid = $('#new-step-fields').validate(false, validationSettings);
            if(!isValid) {
                e.stopPropagation();
...
// Provides functionality to append new rows to the Steps table and hide the modal form for adding new Steps.
...
// Provides functionality for building the HTML that represents a new <TR> for the Steps table.
